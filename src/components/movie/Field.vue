        ...
        <van-tab v-for="(value, key) in film.showMap" :title="key" :key="value.id">
          <div class="fd-item" v-for="val in value" :key="val.id" v-show="val.id">
            ...
            <div class="fd-item-right on">
              <span
                v-if="val.seckillPrice"
                v-html="'<del><price>¥</price>' + priceFormat(val.salesPrice) + '</del> <price>¥</price>' + priceFormat(val.seckillPrice)"
              ></span>
              <span v-else v-html="'<price>¥</price>' + priceFormat(val.salesPrice)"></span>
...
export default {
  ...
  created() {
    ...
    // 影片
    let localFilm = JSON.parse(
      window.localStorage.getItem(this.$init.key.cinemaSelectedFilm)
    );
    if (localFilm) {
      this.film = localFilm;
    }
  ...
  methods: {
    ...
    clickSeat(val) {
      ...
      window.localStorage.setItem(
        this.$init.key.cinemaSelectedShow,
        JSON.stringify(val)
      );
    },
