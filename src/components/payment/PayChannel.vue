    ...
    confirmPayment() {
      ...
      if ("" != vm.selectedPayChannel.payChannelCategory) {
        ...
        if ("1" == vm.selectedPayChannel.payChannelCategory) {
          if (
            vm.$init.channelCategory.WAP_SECKILL.code == vm.orderPay.business
          ) {
            vm.$toast("当前业务不支持此支付方式！");
            return;
          } else {
        ...
        // 微信支付
          ...
          if (
            vm.$init.channelCategory.WAP_CINEMA.code == vm.orderPay.business
          ) {
            vm.wechatPayment("/panda-cinema-api/v1/payment");
          } else if (
            vm.$init.channelCategory.WAP_SECKILL.code == vm.orderPay.business
          ) {
            vm.wechatPayment("/panda-activity-seckill/v1/paymentOrder");
          }
        }
        // 中国邮政储蓄银行
